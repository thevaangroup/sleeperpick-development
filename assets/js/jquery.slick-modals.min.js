...
$(document).ready(function () {
    ...
    // only activate modal on ipad & up
    if(screen.width >= 768){

        $("#popup-modal").slickModals({
            // Functionality
            popupType: "delayed",
            delayTime: 300,
            exitTopDistance: 40,
            ...
            // Overlay options
            //overlayBg: true,
            //overlayBgColor: "url('img/modal-bg.jpg') no-repeat center center / cover",
            ...
            // background color from rgba(31,31,31,1)
            windowBackground: "rgba(255,255,255,1)",
            windowRadius: "0px",
            // changed margin from 30px
            windowMargin: "auto 0px auto 2000px",
            // changed padding from 30px
            windowPadding: "20px",
        ...
        // attach featured artists carousel
        $('overlay').appendTo( $('#profile-container') );


        // toggle .featured artist when slickModal is activated
        $('.openSlickModal-1').click( function(event) {
            $('#profile-container').toggle() 
            // show the first entry
            $(".entry:first-child").fadeIn("slow");
            $(".entry:first-child").addClass("active");
            event.stopPropagation();
        });


        // fadeOut the entry on .closeModal click
        $('.closeModal').click( $('.entry.active').fadeOut("slow") );


        // make sure modal starts in active state on home page
        if ((window.location.pathname === "/") || (window.location.pathname.includes("/index")) ) { 
            $('.openSlickModal-1').trigger( "click" );
        }
        ...
        **/
        ...
        // attach previous event listener
        $(".previous").click(function() {
            //console.log('previous button clicked');

            // fetch current index
            cur = $(".entry").index( $(".entry.active") );
            //console.log('cur index: ',cur);

            // calculate next index
            previous = (cur === 0) ? $(".entry").length-1 : cur - 1;
            //console.log('previous index is: ', previous);
            ...
            // activate next entry
            $(".profile-container").fadeIn();
            $(".entry").eq(parseInt(next)).show();
            $(".entry").eq(parseInt(previous)).addClass("active");
        });

        // attach next event listener
        $(".next").click(function() {
            ...
            // calculate next index
            next = (cur === $(".entry").length-1) ? 0 : cur + 1;
            ...
            $(".profile-container").fadeIn();
            $(".entry").eq(parseInt(next)).show();
            $(".entry").eq(parseInt(next)).addClass("active");
        });


        /**
        *
        *   activating .speech-bubble
        *
        **/
        // .fab:hover
        $('.fab').hover( function(event) {
            $('.entry.active .speech-bubble').fadeIn();
            event.stopPropagation();
            $('.fab-container').hover( function(event) {
                $('.entry.active .speech-bubble').show();
                event.stopPropagation();
            },
            function(event) {
                $('.entry.active .speech-bubble').fadeOut();
                event.stopPropagation();
            });
        });
    }
    else {
        $("footer .fab-container").remove();
    }
